...
Open http://localhost:3000 — you'll land on `/login`. There is only one account type — participant — and the app seeds a single demo account into the `users` table on first run: **participant@pilot.rta.gov.ae**, password **`RtaDemo#2026`** (override with `DEMO_ACCOUNT_PASSWORD` in `.env.local` before first run if you want a different one). Sign-in is real (email + password, checked server-side against a hashed password, session cookie signed and `httpOnly`) — but there's still no self-service sign-up and no real SSO.

**Note:** this seed account is only inserted the first time the `users` table is empty. If this Supabase project already has the older, multi-role demo accounts seeded, apply `supabase/migrations/0005_single_participant_role.sql` to drop the retired roles and their tables.
